...
export const cleanupEmptyRoles = async (
  client: Client,
  ...
  const sortedRoles = [...deletableRoles.values()].sort((a, b) =>
    a.name.localeCompare(b.name),
  );
  const previewNames = sortedRoles
  ...
  if (!dryRun) {
    for (const role of sortedRoles) {
      try {
        await role.delete("Server Admin Console: remove empty role");
        deletedRoleCount += 1;
      } catch (error) {
...
const collectRoleMemberCounts = async (
  client: Client,
  guildId: string,
